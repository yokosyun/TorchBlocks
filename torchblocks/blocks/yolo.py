...
class RepVGGBlock(nn.Module):
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        if self.alpha:
            return self.nonlinearity(
                self.se(
                    self.rbr_dense(inputs) + self.alpha * self.rbr_1x1(inputs) + id_out
                )
            )
        else:
            return self.nonlinearity(
    ...
    def get_equivalent_kernel_bias(self):
        ...
        if self.alpha:
            return (
                kernel3x3
                + self.alpha * self._pad_1x1_to_3x3_tensor(kernel1x1)
                + kernelid,
...
class PPYOLOESELayer(nn.Module):
    ...
    def __init__(
        ...
        self.conv = ConvModule(
            feat_channels, feat_channels, 1, norm_cfg=norm_cfg, act_cfg=act_cfg
        )
...
#         self.conv1 = ConvModule(
#             conv1_in_channels, blocks_channels, 1, norm_cfg=norm_cfg, act_cfg=act_cfg
#         )

#         self.conv2 = ConvModule(
#             conv2_in_channels, blocks_channels, 1, norm_cfg=norm_cfg, act_cfg=act_cfg
#         )

#         self.blocks = self.build_blocks_layer(blocks_channels)

#         self.conv3 = ConvModule(
#             conv3_in_channels, out_channels, 1, norm_cfg=norm_cfg, act_cfg=act_cfg
#         )
...
#                 *(MODELS.build(block_cfg) for _ in range(num_blocks - 1))
#             )

#         if bottle_block == BottleRep:
#             self.conv1 = BottleRep(
#                 in_channels, out_channels, block_cfg=block_cfg, adaptive_weight=True
#             )
...
class CSPSPPFBottleneck(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if isinstance(self.kernel_sizes, int):
            ...
            x3 = torch.cat([x0] + [pooling(x0) for pooling in self.poolings], dim=1)
...
def elan_e100(channel, height, width):
    return ELANBlock(
        in_channels=channel,
...
def elan_e90(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.9,
...
def elan_e875(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=7 / 8,
...
def elan_e80(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.8,
...
def elan_e75(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.75,
...
def elan_e70(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.7,
...
def elan_e625(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=5 / 8,
...
def elan_e60(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.6,
...
def elan_e50(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.5,
...
def elan_e40(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.4,
...
def elan_e375(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=3 / 8,
...
def elan_e30(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.3,
...
def elan_e25(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.25,
...
def elan_e20(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.2,
...
def elan_e10(channel, height, width):
    return ELANBlock(
        in_channels=channel,
        out_channels=channel,
        middle_ratio=0.1,
...
def eelan(channel, height, width):
    # TODO(yoko) check whether implementation is correct or not?
    return EELANBlock(
        in_channels=channel,
        out_channels=channel,
        num_elan_block=2,
...
def cspsspfbottleneck(channel, height, width):
    return CSPSPPFBottleneck(
        in_channels=channel,
        out_channels=channel,
        kernel_sizes=5,
        use_conv_first=True,
        mid_channels_scale=0.5,
        conv_cfg=None,
