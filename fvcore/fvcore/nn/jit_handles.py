...
def scaled_dot_product_attention_flop_jit(
    ...
    input_shapes = [get_shape(v) for v in inputs]
    # assert len(input_shapes) == 7, input_shapes
    # assert input_shapes[0][-1] == input_shapes[1][-2], input_shapes
    k, q, v = input_shapes[0], input_shapes[1], input_shapes[2]

    # k, q, v = inputs[0], inputs[1], inputs[2]

    # q = q.reshape(-1, q[-2], q[-1])
    # k = k.reshape(-1, k[-2], k[-1])
    # v = v.reshape(-1, v[-2], v[-1])
    B, L, E = q[:-2], q[-2], q[-1]
    S, E = k[-2], k[-1]
    S, Ev = v[-2], v[-1]
    ...
    flop = (flop_qk + scale + mask + add_bias + softmax + dropout + flop_attn) * B

    return flop


def zero_flop_jit(inputs: List[Any], outputs: List[Any]) -> Number:
    return 0
